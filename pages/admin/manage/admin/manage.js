...
Page({
  ...
  rejectResident() {
    ...
    const id = this.data.currentResident._id;
    const residents = this.data.residents.map(r => {
      if (r._id === id) return { ...r, status: 'rejected', rejectReason: this.data.rejectReason };
      return r;
    });
  ...
  submitSuggestionReply() {
    ...
    const id = this.data.currentSuggestion._id;
    const suggestions = this.data.suggestions.map(s => {
      if (s._id === id) return { ...s, reply: this.data.suggestionReply, status: 'done' };
      return s;
    });
