...
exports.main = async (event, context) => {
  ...
  try {
    if (action === 'list') {
      const totalResult = await db.collection('announcements').where({ communityId: cid }).count();
      const total = totalResult.total;
      const { data: list } = await db.collection('announcements')
    ...
    if (action === 'update') {
      ...
      await db.collection('announcements').doc(targetId).update({ data: updateData });
      return { success: true };
    }

    if (action === 'delete') {
      if (!targetId) return { success: false, errMsg: '参数缺失' };
      await db.collection('announcements').doc(targetId).remove();
      return { success: true };
    }

    return { success: false, errMsg: '无效操作' };
  } catch (err) {
    console.error('公告管理操作失败', err);
